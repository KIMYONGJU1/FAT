...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.gsp_text_widgets: Dict[str, tk.Text] = {}
        ...
        info_label = ttk.Label(gsp_wrap, text="NAME PLATE (MCCB) 표에서 추출된 회로 목록을 확인/수정하세요.")
        ...
        for idx, (panel_label, placeholder) in enumerate(self.gsp_function_slots):
            ...
            tree = ttk.Treeview(tree_holder, columns=("code", "name"), show="headings", height=8)
            ...
            ttk.Label(frame, text="템플릿 전송용 (CODE + 설명)").grid(row=1, column=0, sticky="w", pady=(10, 2))
            text = tk.Text(frame, height=7, wrap="word")
            text.grid(row=2, column=0, sticky="nsew")
            txt_scroll = ttk.Scrollbar(frame, orient="vertical", command=text.yview)
            txt_scroll.grid(row=2, column=1, sticky="ns")
            text.configure(yscrollcommand=txt_scroll.set)
            self.gsp_text_widgets[placeholder] = text

        ttk.Button(gsp_wrap, text="적용(모든 항목)", command=self._gsp_apply).grid(row=2, column=1, sticky="e", pady=(10, 0))
    ...
    def _validate_template_worker(self):
        ...
        try:
            ...
            for key in FIELDS + FUNCTION_TEST_KEYS:
    ...
    def _gsp_parse_lines(self, text: str) -> List[Dict[str, object]]:
        rows: List[Dict[str, object]] = []
        if not text:
            return rows
        pattern = re.compile(r'(P3[12]-\d{3}-\d{2}-PN)', re.I)
        order = 0
        for line in text.splitlines():
            ...
            m = pattern.match(upper_line)
            if not m:
                continue
            code = m.group(1).upper()
            tail = clean[m.end():].strip(" :-·\t")
            tail = re.sub(r'\s+', ' ', tail)
            rows.append({"code": code, "name": tail, "order": order})
            order += 1
        return rows
    ...
    def _gsp_collect_from_ui(self) -> List[Dict[str, object]]:
        ...
        if not self.gsp_text_widgets:
            return list(existing.values())
        results: List[Dict[str, object]] = []
        for label, placeholder in self.gsp_function_slots:
            widget = self.gsp_text_widgets.get(placeholder)
            content = widget.get("1.0", "end").strip() if widget else ""
            rows = self._gsp_parse_lines(content) if content else []
            if not content and not rows:
                entry = existing.get(placeholder)
                if entry:
                    content = entry.get("content", "") or ""
                    rows = entry.get("rows") or []
            if content or rows:
                sorted_rows = sorted(rows, key=lambda r: r.get("order", 0))
                results.append({
                    "panel": label,
    ...
    def _gsp_refresh_ui(self):
        ...
        for item in self.gsp_function_data:
            ...
            if key:
                data_map[key] = item
        for label, placeholder in self.gsp_function_slots:
            text_widget = self.gsp_text_widgets.get(placeholder)
            if text_widget:
                text_widget.delete("1.0", "end")
                value = data_map.get(placeholder, {}).get("content") or ""
                if value:
    ...
    def _gsp_content_for(self, placeholder: str) -> str:
        for item in self.gsp_function_data:
            key = item.get("placeholder") or self._gsp_placeholder_for_label(item.get("panel", ""))
            if key == placeholder:
    ...
    def _gsp_apply(self):
        self.gsp_function_data = self._gsp_collect_from_ui()
        self._gsp_refresh_ui()
        names = ", ".join(item.get("panel", "") for item in self.gsp_function_data) or "없음"
        self.log(f"[OK] Function Test 업데이트: {names}")
