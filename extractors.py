...
def _parse_colorplate_comprehensive(text: str) -> Dict[str, Dict[str, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        if found_code in name:
            name = name.replace(found_code, '').strip()
        ...
        if found_code:
            ...
            if not name_val:
                if name_col is None and color_col is not None and code_col is not None and color_col != code_col:
                    ...
                    if between_parts:
                        name_val = re.sub(r'\s+', ' ', ' '.join(between_parts)).strip()
            ...
            while lookahead_idx < len(table):
                ...
                if extra_parts:
                    extra = re.sub(r'\s+', ' ', ' '.join(extra_parts)).strip()
                    extra = extra.replace(found_code, '').strip()
                    if extra:
...
def _parse_emergency_circuit_table(
    ...
    for idx, row in enumerate(table[:20]):
        ...
        if 'CIR' in row_text or 'CIRCUIT' in row_text or 'NAME PLATE' in row_text:
            ...
            for col_idx, cell in enumerate(row):
                ...
                if re.search(r"EM'?CY|EMERGENCY", cell_upper) and 'STOP' in cell_upper:
                    emcy_col = col_idx
    ...
    for row_idx in range(header_idx + 1, len(table)):
        ...
        if doc_type == "GSP" and emcy_col is not None and emcy_col < len(row):
            emcy_text = str(row[emcy_col] or '').strip().upper()
            for pattern in emergency_code_patterns:
                match = pattern.search(emcy_text)
                if match:
        ...
        if not code_from_remarks and remarks_col is not None and remarks_col < len(row):
