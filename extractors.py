...
def parse_function_test_of_gsp(pdf_path: str) -> List[Dict[str, object]]:
    ...
    def detect_name_column(lines: List[List[Dict[str, Any]]]) -> Optional[float]:
        for words in lines[:10]:
            text = " ".join(w.get("text", "") for w in words).upper()
            if "CIRCUIT" in text and "NAME" in text:
                after = False
                for w in words:
                    token = (w.get("text", "") or "").upper()
                    if "CIRCUIT" in token and not after:
                        after = True
                        continue
                    if after:
                        return w.get("x0", 0) - 1
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages:
                ...
                if not lines:
                    continue
                name_col = detect_name_column(lines)
